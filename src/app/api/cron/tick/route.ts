  ...
  for (const project of projects) {
    try {
      ...
      if (!pending || pending.length === 0) {
        results.push({ project: project.id, synced: newSessions.length, analyzed: 0, issues: 0 })
        continue
      }
      ...
      results.push({ project: project.id, synced: newSessions.length, analyzed: pending.length, issues: totalIssues })
    } catch (err) {
